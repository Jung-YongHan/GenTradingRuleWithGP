...
from bt4.core.Interceptors import HandleType, CompositeStgyHdgeInterceptor
...
def startSimulation(result_dic, simul_num):
    ...
    si = CompositeStgyHdgeInterceptor(HandleType.Instant_Stream)
...
class MultiProcessor_CompositeStgy_Hdg(unittest.TestCase):
    ...
    def test_bull_trader_multiple_strategies(self):
        ...
        confs = [f'{cfg_prefix}composite_bull_bear_market_hdg']
        ...
        bull_strategy_confs = ['sws_day_hdg_vol', 'sws_ma_crossover_hdg_vol', 'swkim_1_hdg_vol']
        bear_Strategy_confs = ['sws_day_hdg_vol', 'sws_ma_crossover_hdg_vol', 'swkim_1_hdg_vol']
        # bull_strategy_confs = ['sws_day_hdg_vol', 'sws_ma_crossover_hdg_vol']
        # bear_Strategy_confs = ['sws_day_hdg_vol', 'sws_ma_crossover_hdg_vol']
        ...
        self.file_name = join(root_dir, f'report/multi_exec_{nowDatetime}_composite_multi_processor')
                                            ...
                                            for timeframes in timeframes_list:
                                                if ntask % max_task != 0 :
                                                    ...
                                                    dataframe = dataframe.drop_duplicates(['conf', 'simul_start', 'simul_end', 'markets',
                                                                                           'vol_target','bull_strategy_conf', 'bear_strategy_conf',
                                                                                           'decisions', 'timeframes', 'timegap'])
